...
function getNews() {
  return new Promise(resolve => {
    ...
    let n = 6;
    if (newsNum < 6) {
      //如果需要学习的新闻数量不到6，也就是已经学过了，但是积分不够，补的
      n = newsNum;
    }
...
function getVideos() {
  return new Promise(resolve => {
    ...
    let n = 6;
    if (videoNum < 6) {
      //如果需要学习的视频数量不到6，也就是已经学过了，但是积分不够，补的
      n = videoNum;
    }
    ...
    await getExamPaperByPageNo(examPaperPageNo).then(async data => {
      if (data) {
        ...
        if (!continueFind) {
          ...
          if (examPaperTotalPageCount == null || examPaperPageNo > examPaperTotalPageCount || examPaperPageNo < 1) {
    ...
    await getExamWeeklyByPageNo(examWeeklyPageNo).then(async data => {
      if (data) {
        ...
        if (!continueFind) {
          ...
          if (examWeeklyTotalPageCount == null || examWeeklyPageNo > examWeeklyTotalPageCount || examWeeklyPageNo < 1) {
            //已经找完所有页码，还是没找到，不再继续查找
            continueFind = false;
          }
        }
...
function createStartButton() {
  ...
  var baseInfo = "";
  baseInfo +=
    '<form id="settingData" class="egg_menu" action="" target="_blank" onsubmit="return false"><div class="egg_setting_box"><div class="egg_setting_item"><label>新闻</label><input class="egg_setting_switch" type="checkbox" name="0" ' +
    (settings[0] ? "checked" : "") +
    '/>				</div>				<div class="egg_setting_item">					<label>视频</label>					<input class="egg_setting_switch" type="checkbox" name="1" ' +
    (settings[1] ? "checked" : "") +
    '/>				</div>				<div class="egg_setting_item">					<label>每日答题</label>					<input class="egg_setting_switch" type="checkbox" name="6" ' +
    (settings[6] ? "checked" : "") +
    '/>				</div>				<div class="egg_setting_item">					<label>每周答题</label>					<input class="egg_setting_switch" type="checkbox" name="2" ' +
    (settings[2] ? "checked" : "") +
    '/>				</div>				<div class="egg_setting_item">					<label>专项练习</label>					<input class="egg_setting_switch" type="checkbox" name="5" ' +
    (settings[5] ? "checked" : "") +
    '/></div><hr /><div title=\'Tip:开始学习后，隐藏相关页面和提示（不隐藏答题中的关闭自动答题按钮）\' class="egg_setting_item"> <label>运行隐藏</label> <input class="egg_setting_switch" type="checkbox" name="7"' +
    (settings[7] ? "checked" : "") +
    '/></div><a style="text-decoration: none;" title="视频不自动播放？点此查看解决办法" target="blank" href="https://docs.qq.com/doc/DZllGcGlJUG1qT3Vx"><div style="color:#5F5F5F;font-size:14px;" class="egg_setting_item"><label style="cursor: pointer;">视频不自动播放?</label></div></a></div></form>';
